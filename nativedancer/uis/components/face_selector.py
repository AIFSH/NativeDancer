...
from nativedancer.vision import get_video_frame, read_static_image, normalize_frame_color
from nativedancer.face_analyser import get_many_faces
from nativedancer.face_reference import clear_face_reference
from nativedancer.typing import Frame, FaceSelectorMode
...
def render() -> None:
	...
	{
		'label': helperdoc.get('reference_face_gallery_label'),
		'object_fit': 'cover',
		'columns': 8,
		'allow_preview': False,
		'visible': 'reference' in nativedancer.globals.face_selector_mode
	}
	if is_image(nativedancer.globals.target_path):
		reference_frame = read_static_image(nativedancer.globals.target_path)
	...
	FACE_SELECTOR_MODE_DROPDOWN = gradio.Dropdown(
		label = helperdoc.get('face_selector_mode_dropdown_label'),
		choices = nativedancer.choices.face_selector_modes,
		value = nativedancer.globals.face_selector_mode
	)
	REFERENCE_FACE_POSITION_GALLERY = gradio.Gallery(**reference_face_gallery_args)
	REFERENCE_FACE_DISTANCE_SLIDER = gradio.Slider(
		label = helperdoc.get('reference_face_distance_slider_label'),
		value = nativedancer.globals.reference_face_distance,
		step = nativedancer.choices.reference_face_distance_range[1] - nativedancer.choices.reference_face_distance_range[0],
		minimum = nativedancer.choices.reference_face_distance_range[0],
		maximum = nativedancer.choices.reference_face_distance_range[-1],
		visible = 'reference' in nativedancer.globals.face_selector_mode
	)
...
def update_face_selector_mode(face_selector_mode : FaceSelectorMode) -> Tuple[gradio.Gallery, gradio.Slider]:
	if face_selector_mode == 'reference':
		nativedancer.globals.face_selector_mode = face_selector_mode
...
def update_reference_face_position(reference_face_position : int = 0) -> None:
	nativedancer.globals.reference_face_position = reference_face_position


def update_reference_face_distance(reference_face_distance : float) -> None:
	nativedancer.globals.reference_face_distance = reference_face_distance


def update_reference_frame_number(reference_frame_number : int) -> None:
	nativedancer.globals.reference_frame_number = reference_frame_number
