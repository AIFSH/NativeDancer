...
from nativedancer.typing import Face, Frame, Update_Process, ProcessMode
from nativedancer.vision import read_image, read_static_image, write_image
from nativedancer.face_helper import warp_face, create_static_mask_frame
from nativedancer.processors.frame import globals as frame_processors_globals, choices as frame_processors_choices

NAME = 'NATIVEDANCER.FRAME_PROCESSOR.FACE_DEBUGGER'
...
def register_args(program : ArgumentParser) -> None:
	program.add_argument('--face-debugger-items', help = helperdoc.get('face_debugger_items_help'), dest = 'face_debugger_items', default = [ 'kps', 'face-mask' ], choices = frame_processors_choices.face_debugger_items, nargs = '+')
...
def debug_face(source_face : Face, target_face : Face, temp_frame : Frame) -> Frame:
	...
	if 'face-mask' in frame_processors_globals.face_debugger_items:
		...
		mask_frame = create_static_mask_frame(crop_frame.shape[:2], 0, nativedancer.globals.face_mask_padding)
