...
from nativedancer.typing import Frame, ModelValue
from nativedancer.vision import get_video_frame, count_video_frame_total, read_image, detect_fps
...
MODELS : Dict[str, ModelValue] =\
{
	'open_nsfw':
	{
		...
		'path': resolve_relative_path('../weights/open_nsfw.onnx')
	}
}
...
def get_content_analyser() -> Any:
	...
	with THREAD_LOCK:
		if CONTENT_ANALYSER is None:
			...
			CONTENT_ANALYSER = onnxruntime.InferenceSession(model_path, providers = nativedancer.globals.execution_providers)
...
def pre_check() -> bool:
	if not nativedancer.globals.skip_download:
		download_directory_path = resolve_relative_path('../weights')
...
def analyse_video(video_path : str, start_frame : int, end_frame : int) -> bool:
	...
	rate = 0.0
	counter = 0
	with tqdm(total = len(frame_range), desc = helperdoc.get('analysing'), unit = 'frame', ascii = ' =') as progress:
