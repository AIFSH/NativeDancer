...
from nativedancer.face_cache import get_faces_cache, set_faces_cache
from nativedancer.face_helper import warp_face, create_static_anchors, distance_to_kps, distance_to_bbox, apply_nms
from nativedancer.typing import Frame, Face, FaceAnalyserOrder, FaceAnalyserAge, FaceAnalyserGender, ModelValue, Bbox, Kps, Score, Embedding
from nativedancer.utils import resolve_relative_path, conditional_download
from nativedancer.vision import resize_frame_dimension
...
MODELS : Dict[str, ModelValue] =\
{
	'face_detector_retinaface':
	{
		...
		'path': resolve_relative_path('../weights/face_analyser/retinaface_10g.onnx')
	},
	'face_detector_yunet':
	{
		'url': 'https://github.com/facefusion/facefusion-assets/releases/download/models/yunet_2023mar.onnx',
		'path': resolve_relative_path('../weights/face_analyser/yunet_2023mar.onnx')
	},
	'face_recognizer_arcface_blendface':
	{
		...
		'path': resolve_relative_path('../weights/face_analyser/arcface_w600k_r50.onnx')
	},
	...
	'face_recognizer_arcface_simswap':
	{
		...
		'path': resolve_relative_path('../weights/face_analyser/arcface_simswap.onnx')
	},
	'gender_age':
	{
		'url': 'https://github.com/facefusion/facefusion-assets/releases/download/models/gender_age.onnx',
		'path': resolve_relative_path('../weights/face_analyser/gender_age.onnx')
	}
}
...
def get_face_analyser() -> Any:
	...
	with THREAD_LOCK:
		if FACE_ANALYSER is None:
			if nativedancer.globals.face_detector_model == 'retinaface':
				face_detector = onnxruntime.InferenceSession(MODELS.get('face_detector_retinaface').get('path'), providers = nativedancer.globals.execution_providers)
			if nativedancer.globals.face_detector_model == 'yunet':
				face_detector = cv2.FaceDetectorYN.create(MODELS.get('face_detector_yunet').get('path'), '', (0, 0))
			if nativedancer.globals.face_recognizer_model == 'arcface_blendface':
				face_recognizer = onnxruntime.InferenceSession(MODELS.get('face_recognizer_arcface_blendface').get('path'), providers = nativedancer.globals.execution_providers)
			if nativedancer.globals.face_recognizer_model == 'arcface_inswapper':
				face_recognizer = onnxruntime.InferenceSession(MODELS.get('face_recognizer_arcface_inswapper').get('path'), providers = nativedancer.globals.execution_providers)
			if nativedancer.globals.face_recognizer_model == 'arcface_simswap':
				face_recognizer = onnxruntime.InferenceSession(MODELS.get('face_recognizer_arcface_simswap').get('path'), providers = nativedancer.globals.execution_providers)
			gender_age = onnxruntime.InferenceSession(MODELS.get('gender_age').get('path'), providers = nativedancer.globals.execution_providers)
			FACE_ANALYSER =\
			{
...
def pre_check() -> bool:
	if not nativedancer.globals.skip_download:
		download_directory_path = resolve_relative_path('../weights/face_analyser')
		model_urls =\
		[
...
def extract_faces(frame: Frame) -> List[Face]:
	face_detector_width, face_detector_height = map(int, nativedancer.globals.face_detector_size.split('x'))
	...
	elif nativedancer.globals.face_detector_model == 'yunet':
...
def detect_with_retinaface(temp_frame : Frame, temp_frame_height : int, temp_frame_width : int, face_detector_height : int, face_detector_width : int, ratio_height : float, ratio_width : float) -> Tuple[List[Bbox], List[Kps], List[Score]]:
	...
	for index, feature_stride in enumerate(feature_strides):
		keep_indices = numpy.where(detections[index] >= nativedancer.globals.face_detector_score)[0]
		if keep_indices.any():
...
def detect_with_yunet(temp_frame : Frame, temp_frame_height : int, temp_frame_width : int, ratio_height : float, ratio_width : float) -> Tuple[List[Bbox], List[Kps], List[Score]]:
	...
	face_detector.setScoreThreshold(nativedancer.globals.face_detector_score)
	bbox_list = []
	kps_list = []
	score_list = []
	with THREAD_SEMAPHORE:
...
def create_faces(frame : Frame, bbox_list : List[Bbox], kps_list : List[Kps], score_list : List[Score]) -> List[Face] :
	faces : List[Face] = []
	if nativedancer.globals.face_detector_score > 0:
